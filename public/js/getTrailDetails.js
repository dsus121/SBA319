  ...
  item.addEventListener("click", function () {
    ...
    fetch(`/trail/${value}`)
      ...
      .then((trail) => {
        ...
        if (trail) {
          // store the selected trail id - tried not to declare a new variable, didn't work
          ...
          trailDescription.textContent = `description: ${
  ...
  fetch(`/user/${userId}/todos`)
    ...
    .then((todos) => {
      ...
      todos.forEach((todo) => {
        ...
        todoItem.textContent = `${todo.trail_id.name} - ${
          todo.notes || "no notes"
        }`;
...
document
  .getElementById("planTrailButton")
